...
export const assessmentRepo = {
  ...
  patch: async (id: string, data: StepUpdate) => {
    const patch = mapPatch(data);
    const operations: Prisma.PrismaPromise<unknown>[] = [];

    if (Object.keys(patch).length > 0) {
      operations.push(prisma.assessment.update({ where: { id }, data: patch }));
    }
    ...
    return prisma.assessment.findUniqueOrThrow({ where: { id }, select: progressSelect });
  },
